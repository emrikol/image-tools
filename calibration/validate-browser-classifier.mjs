...
const SETS = { photo: 'test-images/kodak', illustration: 'test-images/illustrations', 'line-art': 'test-images/line-art' };

function entropyOf(path) {
  const raw = execFileSync('magick', [path, '-resize', '512x512', '-colorspace', 'sRGB', '-depth', '8', 'RGB:-'], { maxBuffer: 1 << 28 });
  const hist = new Array(256).fill(0); let n = 0;
  for (let i = 0; i + 2 < raw.length; i += 3) {
    hist[Math.min(255, Math.round(0.299 * raw[i] + 0.587 * raw[i + 1] + 0.114 * raw[i + 2]))]++; n++;
  }
  let H = 0;
  for (const c of hist) if (c) { const p = c / n; H -= p * Math.log2(p); }
  return H / 8;
}
...
for (const [type, dir] of Object.entries(SETS)) {
  ...
  data[type] = readdirSync(abs).filter(f => /\.(png|jpe?g)$/i.test(f)).map(f => entropyOf(join(abs, f)));
}
const stat = (v) => { const s = [...v].sort((a, b) => a - b); const q = (p) => s[Math.floor(s.length * p)].toFixed(3);
...
const photo = data.photo, illus = data.illustration;
let best = { acc: 0, th: 0 };
for (let th = 0.30; th <= 0.98; th += 0.002) {
  const acc = (photo.filter(v => v >= th).length + illus.filter(v => v < th).length) / (photo.length + illus.length);

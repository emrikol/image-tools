...
import { jpegQualityFromBuffer, jpegMeta } from './jpeg-quality.mjs';
import { interpolate } from './curves.mjs';

const execFileAsync = promisify(execFile);
const PKG_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const CURVES_DIR = join(PKG_ROOT, 'curves'); // the shipped {metric}-calibration-{type}.json files
...
export async function detectJpegQuality(path) {
  let q = null;
  try {
    q = jpegQualityFromBuffer(readFileSync(path));
  } catch {}
  if (q !== null) return q;
...
export function loadCalibrations(
  type,
  { calibrationDir = CURVES_DIR, ssimOnly = false, extraCalibration = null } = {},
) {
  ...
  if (ssimOnly) {
    const p = existsSync(join(calibrationDir, `ssimulacra2-calibration-${type}.json`))
      ? `ssimulacra2-calibration-${type}.json`
      : `ssimulacra2-calibration-photo.json`;
    const c = read(p);
    return c ? [c] : [];
  }
  const loaded = [];
  for (const f of readdirSync(calibrationDir).filter((f) =>
    f.endsWith(`-calibration-${type}.json`),
  )) {
    const c = read(f);
    if (c) loaded.push(c);
  }
  if (extraCalibration && existsSync(extraCalibration)) {
    const c = read(extraCalibration, '');
    if (c) loaded.push({ ...c, path: extraCalibration });
  }
  return loaded;
}
...
export async function convert(input, opts = {}) {
  const {
    ...
    ssimOnly = false,
    dryRun = false,
    calibrationDir = CURVES_DIR,
    extraCalibration = null,
    onProgress = () => {},
  } = opts;
  ...
  // 1. JPEG quality
  let meta = { components: 3, orientation: 1 };
  try {
    meta = jpegMeta(readFileSync(input));
  } catch {}
  const jpegQ = await detectJpegQuality(input);
  if (jpegQ === null) {
    const e = new Error(`Not a readable JPEG: ${input}`);
    e.code = 'ENOTJPEG';
    throw e;
  }
  onProgress({ type: 'quality', jpegQ });
  const needsNormalize = meta.components === 4 || meta.orientation > 1; // CMYK or EXIF-rotated
  ...
  if (curves.length === 0) {
    const e = new Error(`No calibration curves for "${contentType}"`);
    e.code = 'ENOCURVES';
    throw e;
  }
  let calibWebP = 1,
  ...
  calibWebP = clampQ(calibWebP);
  calibAVIF = clampQ(calibAVIF);
  ...
  if (dryRun) {
    return {
      ...
      jpegQ,
      dryRun: true,
      mode: verify ? 'verify' : 'fast',
      floor: verify ? floor : null,
      curves: perCurve,
      calibWebP,
      calibAVIF,
      origSize,
  ...
  try {
    // cwebp/avifenc can't read CMYK, and may not bake EXIF orientation — normalize via magick to
    // an upright sRGB PNG when needed. Common (RGB, upright) JPEGs skip this entirely.
    let encodeSrc = input;
    if (needsNormalize) {
      const why = meta.components === 4 ? 'CMYK→sRGB' : `EXIF orientation ${meta.orientation}`;
      onProgress({ type: 'normalize', why });
      encodeSrc = join(TMP, 'normalized.png');
      await exec('magick', [input, '-auto-orient', '-colorspace', 'sRGB', encodeSrc]);
      if (!existsSync(encodeSrc)) {
        const e = new Error(
          `Input needs normalizing (${why}) but ImageMagick is unavailable/failed`,
        );
        e.code = 'ENEEDMAGICK';
        throw e;
      }
    ...
    if (!verify) {
      ...
      await encodeWebP(encodeSrc, calibWebP, 50, 60, wout);
      webpResults.push({
        quality: calibWebP,
        sns: 50,
        filter: 60,
      ...
      await encodeAVIF(encodeSrc, calibAVIF, aout);
      avifResults.push({
        quality: calibAVIF,
        sns: null,
        filter: null,
      ...
      const avif = await lowest('avif', async (q) => {
        const out = join(TMP, `a${q}.avif`);
        await encodeAVIF(encodeSrc, q, out);
        return {
          quality: q,
          sns: null,
          filter: null,
          size: fileSize(out),
          score: await measureQuality(encodeSrc, out),
          path: out,
        };
      });
      if (avif.score === null) {
        const e = new Error('Cannot measure (ssimulacra2 missing?)');
        e.code = 'ENOSSIM';
        throw e;
      }
      avifResults.push(avif);
      const webp = await lowest('webp', async (q) => {
        const out = join(TMP, `w${q}_50_60.webp`);
        await encodeWebP(encodeSrc, q, 50, 60, out);
        return {
          quality: q,
          sns: 50,
          filter: 60,
      ...
      if (webp.met) {
        for (const sns of WEBP_SNS)
          for (const f of WEBP_FILTER) {
            ...
            await encodeWebP(encodeSrc, webp.quality, sns, f, out);
            const score = await measureQuality(encodeSrc, out);
    ...
    const withBuf = (r) =>
      r
        ? {
            ...
            score: r.score,
            met: r.met,
            buffer: readFileSync(r.path),
          }
        : null;
    const webp = withBuf(bWebP),
      avif = withBuf(bAVIF);
    ...
    return {
      ...
      webp,
      avif,
      webpCandidates: webpResults.map(({ quality, sns, filter, size, score }) => ({
        quality,
        sns,
        filter,
        size,

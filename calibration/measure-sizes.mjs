...
for (const [type, dir] of Object.entries(TYPES)) {
  const curve = JSON.parse(
    readFileSync(join(ROOT, 'curves', `ssimulacra2-calibration-${type}.json`), 'utf8'),
  ).curve;
  const imgs = readdirSync(join(ROOT, dir))
  ...
  for (const q of QUALS) {
    ...
    out[type].push({
      jpeg_q: q,
      webp_pct: +avg('webp').toFixed(1),
      avif_pct: +avg('avif').toFixed(1),
    });
    process.stderr.write(
      `${type} q${q}: WebP ${avg('webp').toFixed(0)}% / AVIF ${avg('avif').toFixed(0)}% smaller\n`,
    );
  }
}

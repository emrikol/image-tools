...
const namedVals = new Set(
  ['--calibration', '--floor', '--type', '--quality-window', '--ssim-tolerance']
    .map((f) => {
...
if (!INPUT || !existsSync(INPUT)) {
  console.error(
    'Usage: node convert.mjs <input.jpg|dir> [output-dir] [--verify [--floor N]] [--type auto] [--ssim-only] [--keep-both] [--contact-sheet] [--dry-run] [--report]',
  );
  process.exit(1);
}
...
function requireTools() {
  ...
  if (missing.length) {
    ...
    if (missing.includes('ssimulacra2'))
      console.error(
        'ssimulacra2 comes from a libjxl build with devtools; or drop --verify to use fast mode.',
      );
    process.exit(1);
  }
}
...
const convertOpts = {
  ...
  ssimOnly: SSIM_ONLY,
  dryRun: DRY_RUN,
  extraCalibration: EXTRA_CALIBRATION,
};
...
async function runBatch(dir) {
  const files = readdirSync(dir)
    ...
    .sort();
  if (!files.length) {
    console.error(`No .jpg/.jpeg files in ${dir}`);
    process.exit(1);
  }
  requireTools();
  const childFlags = [];
  ...
  const runChild = (inPath) =>
    new Promise((res) => {
      const p = spawn(process.execPath, [self, inPath, OUTPUT_DIR, ...childFlags], {
  ...
  console.log(
    `Batch: ${files.length} JPEG(s) in ${dir}  (${VERIFY ? 'verify' : 'fast'} mode, ${availableParallelism()} parallel)\n`,
  );
  let idx = 0,
    converted = 0,
    kept = 0,
    failed = 0;
  await Promise.all(
    Array.from({ length: Math.min(availableParallelism(), files.length) }, async () => {
      while (idx < files.length) {
        ...
        if (code !== 0) {
          failed++;
          console.log(`  ✗ ${f}  (failed; rerun without batch to see why)`);
        } else if (/keeping the original/i.test(out)) {
          kept++;
          console.log(`  • ${f}  → kept original (no smaller encode)`);
        } else if (win) {
          converted++;
          console.log(`  ✓ ${f}  → ${win[1]}`);
        } else {
...
async function runSingle() {
  ...
  const onProgress = (e) => {
    if (e.type === 'classified') {
      ...
      for (const c of e.curves)
        console.log(
          `${c.metric.padEnd(16)} — WebP: q=${String(c.webp_q ?? '—').padStart(3)}  AVIF: q=${String(c.avif_q ?? '—').padStart(3)}  (${c.path})`,
        );
      if (e.curves.length > 1)
        console.log(`${'Using (max)'.padEnd(16)} — WebP: q=${e.calibWebP}  AVIF: q=${e.calibAVIF}`);
    } else if (e.type === 'mode' && !DRY_RUN) {
      console.log(
        `Mode: ${e.mode === 'verify' ? `verify (floor ${FLOOR.toFixed(1)})` : 'fast (curve-only)'}\n`,
      );
      ...
      if (e.format !== curFmt) {
        ...
        process.stdout.write(`Searching ${e.format.toUpperCase()} `);
        curFmt = e.format;
      }
  ...
  try {
    ...
    if (e.code === 'ENOTJPEG')
      console.error(
        `Could not read a JPEG quantization table from ${basename(INPUT)} — is it a JPEG? (This tool converts JPEGs.)`,
      );
    else if (e.code === 'ENOCURVES')
      console.error(
        `No calibration curves found for this content type — expected {metric}-calibration-*.json beside the script (they ship with the repo).`,
      );
    else if (e.code === 'ENOSSIM')
      console.error(
        'Could not measure encodes — is `ssimulacra2` installed? Omit --verify for fast (curve-only) mode.',
      );
    else if (e.code === 'ENEEDMAGICK')
      console.error(
        `${e.message}. Install ImageMagick (brew install imagemagick) to convert CMYK / EXIF-rotated JPEGs.`,
      );
  ...
  if (r.pixelArt) {
    console.log(
      'Pixel-art detected — lossy encoding not recommended. Use oxipng/optipng on the original PNG instead.',
    );
    return;
  }
  if (r.dryRun) {
  ...
  if (r.webp)
    console.log(
      `Best WebP:      ${kb(r.webp.size)}  (${pct(r.webp.size, r.origSize)} smaller)  q=${r.webp.quality} sns=${r.webp.sns} f=${r.webp.filter}  ${scoreStr(r.webp.score)}`,
    );
  if (r.avif)
    console.log(
      `Best AVIF:      ${kb(r.avif.size)}  (${pct(r.avif.size, r.origSize)} smaller)  q=${r.avif.quality}  ${scoreStr(r.avif.score)}`,
    );

  if (REPORT) {
    console.log('\n── WebP results (sorted by size) ───');
    for (const c of r.webpCandidates.slice(0, 10))
      console.log(
        `  ${kb(c.size).padStart(8)}  q=${c.quality} sns=${c.sns} f=${c.filter}  ${scoreStr(c.score)}`,
      );
    console.log('\n── AVIF results ─────────────────────────────────────────');
    for (const c of r.avifCandidates)
      console.log(`  ${kb(c.size).padStart(8)}  q=${c.quality}  ${scoreStr(c.score)}`);
  }
  ...
  if (KEEP_BOTH) {
    if (r.webp && r.winner !== 'webp') {
      const p = join(OUTPUT_DIR, `${stem}.webp`);
      writeFileSync(p, r.webp.buffer);
      console.log(`  Also saved: ${p}`);
    }
    if (r.avif && r.winner !== 'avif') {
      const p = join(OUTPUT_DIR, `${stem}.avif`);
      writeFileSync(p, r.avif.buffer);
      console.log(`  Also saved: ${p}`);
    }
  }
...
async function exec(cmd, a) {
  try {
    return await execFileAsync(cmd, a, { encoding: 'utf8' }).catch((e) => e);
  } catch {
...
async function buildContactSheet(r, stem) {
  ...
  try {
    ...
    const tiles = [
      {
        path: INPUT,
        label: `ORIGINAL JPEG  ·  q${r.jpegQ}  ·  ${r.contentType}\n${kb(r.origSize)}   (reference${r.floor != null ? `, floor ${r.floor.toFixed(2)}` : ', fast mode'})`,
      },
    ];
    if (r.webp) {
      const p = join(TMP, `${stem}.webp`);
      writeFileSync(p, r.webp.buffer);
      tiles.push({
        path: p,
        label: `WebP  ·  q${r.webp.quality} sns${r.webp.sns} f${r.webp.filter}${r.winner === 'webp' ? '   ✓ WINNER' : ''}\n${kb(r.webp.size)}  (${pct(r.webp.size, r.origSize)} smaller)${ss(r.webp.score)}`,
      });
    }
    if (r.avif) {
      const p = join(TMP, `${stem}.avif`);
      writeFileSync(p, r.avif.buffer);
      tiles.push({
        path: p,
        label: `AVIF  ·  q${r.avif.quality}${r.winner === 'avif' ? '   ✓ WINNER' : ''}\n${kb(r.avif.size)}  (${pct(r.avif.size, r.origSize)} smaller)${ss(r.avif.score)}`,
      });
    }
    ...
    const bodyPath = join(TMP, 'body.png'),
      headerPath = join(TMP, 'header.png');
    ...
    const bodyW =
      parseInt(
        ((await exec('magick', ['identify', '-format', '%w', bodyPath])).stdout || '').trim(),
      ) || 0;
    if (bodyW > 0) {
      ...
      hArgs.push(
        ...
        '-gravity',
        'center',
        `caption:${basename(INPUT)}    ·    perceptually-matched  JPEG / WebP / AVIF    ·    content: ${r.contentType}`,
        headerPath,
      );

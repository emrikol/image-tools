...
export function loadCalibrations(type, { calibrationDir = CURVES_DIR, ssimOnly = false, extraCalibration = null } = {}) {
...
export async function convert(input, opts = {}) {
  const {
    ...
    calibrationDir = CURVES_DIR, extraCalibration = null, onProgress = () => {},
  } = opts;
  ...
  try { meta = jpegMeta(readFileSync(input)); } catch {}
  ...
  try {
    ...
    if (needsNormalize) {
      ...
      if (!existsSync(encodeSrc)) {
        const e = new Error(`Input needs normalizing (${why}) but ImageMagick is unavailable/failed`);
        e.code = 'ENEEDMAGICK'; throw e;
      }
    }
    ...
    if (!verify) {
      ...
      const avif = await lowest('avif', async (q) => {
        const out = join(TMP, `a${q}.avif`); await encodeAVIF(encodeSrc, q, out);
        return { quality: q, sns: null, filter: null, size: fileSize(out), score: await measureQuality(encodeSrc, out), path: out };
      });
      ...
      const webp = await lowest('webp', async (q) => {
        const out = join(TMP, `w${q}_50_60.webp`); await encodeWebP(encodeSrc, q, 50, 60, out);
        return { quality: q, sns: 50, filter: 60, size: fileSize(out), score: await measureQuality(encodeSrc, out), path: out };
      });

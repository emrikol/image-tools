...
const METRIC_NAMES = getArg('--metrics', 'ssimulacra2')
  .split(',')
  .map((s) => s.trim());
...
const datasetSpecs = getAllArgs('--dataset').map((spec) => {
  ...
  if (parts.length < 3) {
    console.error(`Invalid --dataset: "${spec}" — expected type:image-dir:output-dir`);
    process.exit(1);
  }
...
if (datasetSpecs.length === 0) {
  console.error(
    'Usage: node calibration/calibrate.mjs --dataset type:image-dir:output-dir [--dataset ...] [--metrics m1,m2,...] [--step N] [--concurrency N] [--cache-dir DIR]',
  );
  process.exit(1);
}

// JPEG qualities to sample
const JPEG_QUALITIES = Array.from({ length: 100 }, (_, i) => i + 1).filter(
  (q) => q % STEP === 0 || q === 1,
);
...
async function measureSSIM(ref, encoded) {
  return withPng(encoded, async (cmpPath) => {
    const result = await execFileAsync('ssimulacra2', [ref, cmpPath], { encoding: 'utf8' }).catch(
      (e) => e,
    );
...
async function measureButteraugli(ref, encoded) {
  return withPng(encoded, async (cmpPath) => {
    const result = await execFileAsync('butteraugli_main', [ref, cmpPath], {
      encoding: 'utf8',
    }).catch((e) => e);
    const output = result.stdout || result.stderr || '';
    const match = output.match(/([\d.]+)/);
...
async function measureDSSIM(ref, encoded) {
  return withPng(encoded, async (cmpPath) => {
    const result = await execFileAsync('dssim', [ref, cmpPath], { encoding: 'utf8' }).catch(
      (e) => e,
    );
    const output = result.stdout || result.stderr || '';
...
async function measureXPSNR(ref, encoded) {
  const result = await execFileAsync(
    'ffmpeg',
    ['-hide_banner', '-i', ref, '-i', encoded, '-lavfi', '[0:v][1:v]xpsnr', '-f', 'null', '-'],
    { encoding: 'utf8' },
  ).catch((e) => e);
  const output = result.stdout || result.stderr || '';
...
async function measureVMAF(ref, encoded) {
  ...
  try {
    ...
    await execFileAsync(
      'vmaf',
      ['-r', tmpRef, '-d', tmpDst, '--model', 'version=vmaf_v0.6.1', '--json', '-o', logFile, '-q'],
      { encoding: 'utf8' },
    ).catch((e) => e);
    ...
    for (const f of [tmpRef, tmpDst, logFile]) {
      try {
        unlinkSync(f);
      } catch {}
    }
...
class PyPool {
  constructor(metric, size) {
    this.workers = Array.from({ length: size }, () => new PyWorker(metric));
  }
...
const METRICS = {
  ssimulacra2: {
    ...
    scoreField: 'score',
    higherIsBetter: true,
    async prepareRef(_ref) {
      return {};
    },
    async measure(ref, encoded, _ctx) {
      return measureSSIM(ref, encoded);
    },
  },
  butteraugli: {
    name: 'butteraugli',
    scoreField: 'butteraugli_dist',
    higherIsBetter: false,
    async prepareRef(_ref) {
      return {};
    },
    async measure(ref, encoded, _ctx) {
      return measureButteraugli(ref, encoded);
    },
  },
  dssim: {
    name: 'dssim',
    ...
    async measure(ref, encoded, _ctx) {
      return measureDSSIM(ref, encoded);
    },
  },
  xpsnr: {
    name: 'xpsnr',
    ...
    async measure(ref, encoded, _ctx) {
      return measureXPSNR(ref, encoded);
    },
  },
  cvvdp: {
    name: 'cvvdp',
    scoreField: 'cvvdp_jod',
    higherIsBetter: true,
    async prepareRef(_ref) {
      return {};
    },
    async measure(ref, encoded, _ctx) {
      return measureCVVDP(ref, encoded);
    },
  },
  ms_ssim: {
    name: 'ms_ssim',
    ...
    async measure(ref, encoded, _ctx) {
      return measureMSSSIM(ref, encoded);
    },
  },
  ...
  lpips: {
    ...
    async measure(ref, encoded, _ctx) {
      return measurePython('lpips', ref, encoded);
    },
  },
  dists: {
    name: 'dists',
    ...
    async measure(ref, encoded, _ctx) {
      return measurePython('dists', ref, encoded);
    },
  },
  fsim: {
    name: 'fsim',
    ...
    async measure(ref, encoded, _ctx) {
      return measurePython('fsim', ref, encoded);
    },
  },
  vif: {
    name: 'vif',
    ...
    async measure(ref, encoded, _ctx) {
      return measurePython('vif', ref, encoded);
    },
  },
  entropy_diff: {
    name: 'entropy_diff',
    scoreField: 'entropy_diff',
    higherIsBetter: false,
    async prepareRef(_ref) {
      return {};
    },
    async measure(ref, encoded, _ctx) {
      return measureEntropyDiff(ref, encoded);
    },
  },
};
...
async function measureMetric(m, imageInfos) {
  ...
  for (const info of imageInfos) {
    ...
    for (const q of JPEG_QUALITIES) {
      tasks.push(async () => {
        s.jpeg[q] = await m.measure(info.rgbRef, join(info.imageCache, `jpeg-q${q}.jpg`), ctx);
        measureProgress.tick();
      });
      tasks.push(async () => {
        s.webp[q] = await m.measure(info.rgbRef, join(info.imageCache, `webp-q${q}.webp`), ctx);
        measureProgress.tick();
      });
      tasks.push(async () => {
        s.avif[q] = await m.measure(info.rgbRef, join(info.imageCache, `avif-q${q}.avif`), ctx);
        measureProgress.tick();
      });
    }
  }
  ...
  for (const info of imageInfos) {
    ...
    for (const jpegQ of JPEG_QUALITIES) {
      ...
      rows.push({
        jpeg_q: jpegQ,
        [m.scoreField]: target,
        webp_q: findEquivQuality(
          JPEG_QUALITIES.map((q) => ({ q, score: s.webp[q] })),
          target,
          m.higherIsBetter,
        ),
        avif_q: findEquivQuality(
          JPEG_QUALITIES.map((q) => ({ q, score: s.avif[q] })),
          target,
          m.higherIsBetter,
        ),
      });
...
function averageCurves(imageResults, metricName) {
  ...
  const avg = (v) =>
    v.length ? parseFloat((v.reduce((a, b) => a + b, 0) / v.length).toFixed(4)) : null;
...
for (const spec of datasetSpecs) {
  if (!existsSync(spec.imageDir)) {
    console.error(`Image dir not found: ${spec.imageDir}`);
    process.exit(1);
  }
  ...
  if (images.length === 0) {
    console.error(`No PNG files in ${spec.imageDir}`);
    process.exit(1);
  }
...
console.log(
  `step: ${STEP}  concurrency: ${CONCURRENCY}  avif-speed: ${AVIF_SPEED}${ENCODE_ONLY ? '  [encode-only]' : ''}`,
);
...
for (const m of activeMetrics) {
  ...
  for (const ds of datasets) {
    ...
    for (const row of averaged) {
      ...
      console.log(
        `  ${String(row.jpeg_q).padStart(3)}  │  ${String(sv ?? '—').padStart(m.scoreField.length)}  │  ${String(row.webp_q ?? '—').padStart(4)}  │  ${String(row.avif_q ?? '—').padStart(4)}`,
      );
    }
  }
